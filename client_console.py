...
class Outbound():
    ...
    def __init__(self, name, host_listen='127.0.0.1', port_listen=0000, host_connect='localhost', port_connect=9876):
        ...
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            print('Connecting to server {} and {}'.format(str(host_connect), port_connect))
        ...
        while True:
            ...
            elif input_msg == '?peers':    # Need to type exact trigger
                for conn in incoming_conn:
                    if str(self.port_connect) in str(conn):
                        msg = bytes(str(input_msg), encoding="utf-8")
                        conn.send(msg)
...
def main():
    ...
    while True:
        name_raw = input('Nickname [Max 20 char]: ')
        if name_raw:
            name_raw = name_raw[:20]
            name = name_raw.replace(" ", "_")
            break

    # host_listen = input('Host: ')
    host_listen = '0.0.0.0'

    # port_listen = int(port_listen)
    # port_listen = input('Listen at port: ')
    port_listen = 0

    host_connect = input('Connect to host [Default localhost]: ')

    # Troubleshooting
    # host_connect = 'external IP'  # My external IP
    # host_connect = '127.0.0.1'
    # host_connect = '10.0.2.10'
    # host_connect = '172.0.0.24'

    # port_connect = input('Connect to port: ')
    port_connect = 0

    # For troubleshooting generate names automatically
    # names_list = ['John', 'Paul', 'Carolina', 'Ana', 'Richard', 'Matt', 'Jack', 'Carl', 'Susan']
    # random_number = randint(0, len(names_list)-1)
    # random_number2 = randint(0, 999)
    # name = names_list[random_number]
    # name += '_' + str(random_number2)

    lock = threading.Lock()

    c = threading.Thread(target=Outbound(
        name, host_listen=host_listen, port_listen=port_listen, host_connect=host_connect, port_connect=port_connect),
        args=())
    c.daemon = True
    c.start()
